// You are the Librarian of the Ninja library. There are ‘N’ books available in the library and ‘B’ ninjas
// want to
//  read the books. You know the number of pages in each book and you have to allocate the books to the ‘B’
// ninjas in
...
// Allotment of the books should be done in a contiguous manner. For example, a ninja can not be allocated
// book 2 
...
bool isPossible(vector<int> &pages, int n, int b,int mid){
	...
	for(int i-0; i<n; i++ ){
		if(pages[i] + pagesCount <= pages[mid]){
			pagesCount += pages[i];
		}
		else{
			studentCount++;
